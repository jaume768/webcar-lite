...
def _reservar(economico, palma, **kwargs):
    kwargs.setdefault("start", en(1))
    kwargs.setdefault("end", en(3))
    return reserve_capacity(category=economico, pickup_office=palma, **kwargs)


def test_con_tres_coches_la_cuarta_reserva_solapada_se_rechaza(economico, palma, tres_coches):
    for _ in range(3):
        _reservar(economico, palma)
...
def test_un_vehiculo_bloqueado_no_cuenta_como_capacidad(economico, palma, tres_coches):
...
def test_un_bloqueo_que_no_pisa_el_periodo_no_resta(economico, palma, tres_coches):
...
def test_el_estado_del_vehiculo_no_decide_el_futuro(economico, palma, tres_coches):
    ...
    assert check_category_availability(economico, palma, en(20), en(22)).free == 3


def test_un_vehiculo_de_baja_no_es_flota(economico, palma, tres_coches):
    from apps.fleet.services import set_vehicle_active

    set_vehicle_active(vehicle=tres_coches[0], active=False)

    assert check_category_availability(economico, palma, en(1), en(3)).total_fleet == 2


def test_una_reserva_sin_vehiculo_consume_capacidad(economico, palma, un_coche):
    """La reserva va contra la categoria: no hace falta coche para ocupar hueco."""
    reserva = _reservar(economico, palma)
...
def test_que_estados_ocupan_flota(economico, palma, un_coche, estado, resta):
...
def test_una_reserva_de_otra_categoria_no_estorba(economico, premium, palma, un_coche):
    from apps.fleet.tests.factories import VehicleFactory

    VehicleFactory(plate="5555PRE", category=premium, current_office=palma)
    _reservar(premium, palma)

    assert check_category_availability(economico, palma, en(1), en(3)).available


def test_sin_flota_en_el_grupo_no_hay_disponibilidad(economico, palma):
    resultado = check_category_availability(economico, palma, en(1), en(3))
...
def test_get_available_categories_solo_ofrece_lo_vendible(economico, premium, palma, tres_coches):
    """Premium no tiene coches en el grupo: no se ofrece."""
    disponibles = get_available_categories(palma, en(1), en(3))
...
def test_get_available_categories_puede_devolver_tambien_las_llenas(
    economico, premium, palma, tres_coches
):
    todas = get_available_categories(palma, en(1), en(3), include_unavailable=True)
...
def test_una_categoria_retirada_no_se_ofrece(economico, palma, tres_coches):
    ...
    disponibles = get_available_categories(palma, en(1), en(3), include_unavailable=True)
