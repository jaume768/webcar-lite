...
def test_una_reserva_no_se_cuenta_a_si_misma_al_cambiar_fechas(economico, centro, un_coche):
    ...
    sin_excluir = check_category_availability(economico, centro, en(1), en(5))
    excluyendola = check_category_availability(
        economico, centro, en(1), en(5), exclude_reservation=reserva
    )
...
def test_exclude_reservation_acepta_tambien_la_clave(economico, centro, un_coche):
    reserva = reserve_capacity(category=economico, pickup_office=centro, start=en(1), end=en(3))

    resultado = check_category_availability(
        economico, centro, en(1), en(3), exclude_reservation=reserva.pk
    )

    assert resultado.available


def test_alargar_cuando_el_coche_ya_esta_vendido_a_otro_se_rechaza(economico, centro, un_coche):
...
def test_alargar_sin_conflicto_si_hay_hueco(economico, centro, un_coche):
...
def test_al_cambiar_de_categoria_se_revalida_la_nueva(economico, premium, centro, un_coche):
    reserva = reserve_capacity(category=economico, pickup_office=centro, start=en(1), end=en(3))
...
def test_asignar_un_coche_libre(economico, centro, tres_coches):
    reserva = reserve_capacity(category=economico, pickup_office=centro, start=en(1), end=en(3))
...
def test_asignar_un_coche_ya_comprometido_se_rechaza(economico, centro, tres_coches):
    ...
    otra = reserve_capacity(category=economico, pickup_office=centro, start=en(2), end=en(4))
...
def test_asignar_un_coche_de_otra_categoria_se_rechaza(economico, premium, centro, tres_coches):
    from apps.fleet.tests.factories import VehicleFactory

    ajeno = VehicleFactory(plate="7777PRE", category=premium, current_office=centro)
    reserva = reserve_capacity(category=economico, pickup_office=centro, start=en(1), end=en(3))
...
def test_saltarse_el_servicio_choca_contra_la_base_de_datos(economico, centro, un_coche):
...
def test_una_cancelada_no_bloquea_el_mismo_coche(economico, centro, un_coche):
...
def test_check_vehicle_availability(economico, centro, un_coche):
...
def test_get_available_vehicles_descarta_ocupados_y_bloqueados(economico, centro, tres_coches):
    reserve_capacity(
        category=economico,
        pickup_office=centro,
        start=en(1),
    ...
    libres = list(get_available_vehicles(economico, centro, en(1), en(3)))
...
def test_dar_de_baja_un_coche_deja_sus_reservas_para_reasignar(economico, centro, tres_coches):
...
def test_la_baja_no_toca_el_historico(economico, centro, tres_coches):
...
def test_tras_la_baja_la_reserva_se_puede_reasignar(economico, centro, tres_coches):
    coche = tres_coches[0]
    futura = reserve_capacity(
        category=economico,
        pickup_office=centro,
