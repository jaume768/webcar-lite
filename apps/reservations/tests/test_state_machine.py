...
def _reserva(economico, centro, cliente, **kwargs):
    kwargs.setdefault("category", economico)
    kwargs.setdefault("pickup_office", centro)
    kwargs.setdefault("return_office", centro)
...
def test_todo_salto_fuera_del_diagrama_falla(economico, centro, cliente, jefe, desde, hasta):
    reserva = _reserva(economico, centro, cliente, status=desde)
...
def test_no_se_puede_finalizar_saltandose_en_curso(economico, centro, cliente, jefe, coche):
    """El caso que mas duele: confirmar y dar por terminado sin entregar."""
    reserva = _reserva(
        economico, centro, cliente, status=ReservationStatus.CONFIRMED, vehicle=coche
    )
...
def test_un_estado_final_no_va_a_ninguna_parte(economico, centro, cliente, jefe):
    for final in (
...
def test_una_transicion_sin_permiso_no_deja_rastro(economico, centro, cliente, solo_lectura):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.PENDING)
...
def test_el_agente_confirma_pero_no_cancela(economico, centro, cliente, agente):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.PENDING)
...
def test_available_transitions_depende_del_usuario(economico, centro, cliente, agente, responsable):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.CONFIRMED)
...
def test_confirmar_sin_cliente_se_rechaza(economico, centro, agente):
...
def test_entregar_sin_vehiculo_se_rechaza(economico, centro, cliente, agente):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.CONFIRMED)
...
def test_entregar_sin_check_in_se_rechaza(economico, centro, cliente, agente, coche):
    reserva = _reserva(
        economico, centro, cliente, status=ReservationStatus.CONFIRMED, vehicle=coche
    )
...
def test_entregar_con_todo_en_orden(economico, centro, cliente, agente, coche):
...
def test_finalizar_sin_check_out_se_rechaza(economico, centro, cliente, agente, coche):
    reserva = _reserva(
        economico,
        centro,
        cliente,
...
def test_finalizar_manda_el_coche_a_limpieza(economico, centro, cliente, agente, coche):
...
def test_no_se_entrega_un_coche_que_salio_de_flota(economico, centro, cliente, agente, coche):
    reserva = _reserva(
        economico,
        centro,
        cliente,
...
def test_cancelar_una_reserva_en_curso_exige_permiso_y_motivo(
    economico, centro, cliente, agente, responsable, coche
):
...
def test_cancelar_una_pendiente_no_pide_motivo(economico, centro, cliente, responsable):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.PENDING)
...
def test_el_no_show_exige_motivo(economico, centro, cliente, responsable):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.CONFIRMED)
...
def test_cancelar_aplica_la_politica_y_congela_el_cargo(economico, centro, cliente, responsable):
...
def test_cancelar_con_antelacion_no_cuesta_nada(economico, centro, cliente, responsable):
    reserva = _reserva(
        economico,
        centro,
        cliente,
...
def test_una_no_reembolsable_cuesta_el_total(economico, centro, cliente, responsable):
    reserva = _reserva(
        economico,
        centro,
        cliente,
...
def test_cada_transicion_deja_su_fila(economico, centro, cliente, agente, responsable):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.DRAFT)
...
def test_can_transition_no_miente(economico, centro, cliente, agente):
    reserva = _reserva(economico, centro, cliente, status=ReservationStatus.CONFIRMED)
...
def test_el_estado_se_relee_con_bloqueo(economico, centro, cliente, agente):
